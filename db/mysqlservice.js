...
var mysqlconfig = {
    "host": "127.0.0.1",
    "port": "3306",
    "database": "black-explorerdb",
    "username": "admin",
    "passwd": "password"
}
var config = require('../config.json');
if ( config.mysql ) {
    if ( config.mysql.host ) { mysqlconfig.host = config.mysql.host; }
    if ( config.mysql.port ) { mysqlconfig.port = config.mysql.port; }
    if ( config.mysql.database ) { mysqlconfig.database = config.mysql.database; }
    if ( config.mysql.username ) { mysqlconfig.username = config.mysql.username; }
    if ( config.mysql.passwd ) { mysqlconfig.passwd = config.mysql.passwd; }
} else {
    if ( process.env.MYSQL_HOST ) { mysqlconfig.host = process.env.MYSQL_HOST; }
    if ( process.env.MYSQL_PORT ) { mysqlconfig.port = process.env.MYSQL_PORT; }
    if ( process.env.MYSQL_DATABASE ) { mysqlconfig.database = process.env.MYSQL_DATABASE; }
    if ( process.env.MYSQL_USERNAME ) { mysqlconfig.username = process.env.MYSQL_USERNAME; }
    if ( process.env.MYSQL_PASSWORD ) { mysqlconfig.passwd = process.env.MYSQL_PASSWORD; }
}
